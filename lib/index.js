...
class MockProvider extends ethers.providers.InfuraProvider {
  ...
  async perform(method, params) {
	...
	if(mock.mockedFunctions.get(params.transaction.data) === undefined) {
		return super.perform(method, params)
	}
	const cointerface = mock.interface
	 
	const functionSignature = params.transaction.data
	  
	const fragment = cointerface.getFunction(functionSignature)
	return cointerface.encodeFunctionResult(fragment, [mock.mockedFunctions.get(functionSignature)])
